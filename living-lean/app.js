...
<body>
  <p class="print-tip">🖨 Tap the Share icon, then choose <strong>Print</strong>. When you're done, switch back to Living Lean from your Home Screen or the App Switcher.</p>
  ${bodyHtml}
</body>
</html>`;
...
function printRecipe() {
  ...
  const html = `
    <h1>${r.name}</h1>
    <div class="print-meta">Base: ${baseServings} servings${scaleNote} · Living Lean</div>
    ${storyBlock}
    <h2>Ingredients</h2>
    <table>${ingRows}</table>
    <h2>Steps</h2>
...
function printShoppingList() {
  ...
  const html = `
    ...
    <div class="print-meta">Living Lean${scaleNote}</div>
    <table>${rows}</table>
  `;
...
function printAllFavorites() {
  ...
  favRecipes.forEach(r => {
    ...
    recipesHtml += `<div class="recipe-block">
      <h1>${r.name}</h1>
      <div class="print-meta">${r.baseServings} servings · Living Lean</div>
      ${storyBlock}
      <h2>Ingredients</h2>
      <table>${ingRows}</table>
      <h2>Steps</h2>
  ...
  const html = `
    <div class="print-meta">${favRecipes.length} favorite recipe${favRecipes.length !== 1 ? 's' : ''} · Living Lean</div>
    ${recipesHtml}
  `;
...
async function shareApp() {
  const data = {
    title: 'Living Lean',
    text: 'Check out Living Lean — low-carb, low-cal recipes!',
    url: 'https://dsshrek-ai.github.io/senior-family-cookbook/living-lean'
  };
  if (navigator.share) {
...
async function shareRecipe() {
  ...
  text += `\nFrom Living Lean: https://dsshrek-ai.github.io/senior-family-cookbook/living-lean`;
...
function setupInstallPrompt() {
  window.addEventListener('beforeinstallprompt', e => {
    ...
    banner.innerHTML = `
      <p>Add <strong>Living Lean</strong> to your home screen</p>
